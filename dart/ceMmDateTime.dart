...
class ceDateTime {
  ...
  ceDateTime({m_jd, m_tz, int m_ct = 0, int m_SG = 2361222}) {
    ...
    _m_tz =
        m_tz ?? ceDateTime.ltzoh(); // time zone for this particular instance
...
// convert unix timestamp to jd
  static u2j(ut) {
    ...
    var jd =
        2440587.5 + ut / 86400.0; //converte to day(/24h/60min/60sec) and to JD
    return jd;
  }
    ...
    if (str.length == 8 || str.length == 14 || str.length == 17) {
      ...
      if (str.length == 14 || str.length == 17) {
        ...
        if (str.length == 17) {
          ...
          ls = parseInt(pstr);
          s += Math.round(ls / 1000);
        }
      }
      jd = ceDateTime.w2j(y: y, m: m, d: d, h: h, n: n, s: s, ct: ct, SG: SG) -
          tz / 24.0; // convert to UTC
    }
    return jd;
  }
...
// get properties
  ...
  get jdl {
    return (_m_jd + _m_tz / 24.0);
  } // julian date for this time zone

  get jdn {
    return Math.round(_m_jd);
  } // julian date number

  get jdnl {
    return Math.round(_m_jd + _m_tz / 24.0);
  } // julian date number for this time zone

  get y {
    var dt = ceDateTime.j2w(jd: this.jdl, ct: _m_ct, SG: _m_SG);
    return dt.y;
  } // year
  ...
  get mlen {
    return ceDateTime.wml(y: this.y, m: this.m, ct: _m_ct, SG: _m_SG);
...
//-------------------------------------------------------------------------
  ceMmDateTime({m_jd, m_tz, m_ct = 0, m_SG = 2361222})
      : super(m_jd: m_jd, m_tz: m_tz, m_ct: m_ct, m_SG: m_SG) {}
//-------------------------------------------------------------------------
...
// dependency: GetMyConst(my)
  static cal_watat(my) {
    ...
    var c = ceMmDateTime.GetMyConst(
        my); // get constants for the corresponding calendar era
    var TA = (SY / 12 - LM) * (12 - c.NM); //threshold to adjust
    var ed = (SY * (my + 3739)) % LM; // excess day
    if (ed < TA) ed += LM; //adjust excess days
    var fm = Math.round(
        SY * my + MO - ed + 4.5 * LM + c.WO); //full moon day of 2nd Waso
    var TW = 0.0, watat = 0; //find watat
    if (c.EI >= 2) {
...
// dependency: cal_my(my)
  static m2j(my, mm, md) {
    ...
    mm += 4 -
        Math.floor((mm + 15) / 16) * 4 +
        Math.floor((mm + 12) / 16); //adjust month
    dd = md +
...
// ... )
  SetMDateTime({my, mm, md, hour = 12, minute = 0, second = 0, tz = 0}) {
    _m_jd = ceMmDateTime.m2j(my, mm, md) +
...
// output: ( [1=nagapor, 0=else])
  static cal_nagapor(md, wd) {
    ...
    if (((md == 2) && (wd == 1)) ||
        (((md == 12) || (md == 4) || (md == 18)) && (wd == 2))) nagapor = 1;
    return nagapor;
  }
...
// dependency: w2j()
  static DoE(y) {
    ...
    p = (q % 31) + 1;
    n = Math.floor(q / 31);
    return Math.round(ceDateTime.w2j(
        y: y, m: n, d: p, h: 12, n: 0, s: 0, ct: 1)); // this is for Gregorian
  }
...
// &f : fortnight day [1-15]
  ToMString({fs = "&yyyy &M &P &ff"}) {
    return ceMmDateTime.j2ms(jd: this.jd, fs: fs, tz: this.tz);
  }
} //ceMmDateTime
...
//		3: Mon, 4: Shan, 5: Karen
  T({str, toLn = 1, fromLn = 0}) {
    var i;
    var l = _m_lang.length;
    var fstr, rstr, re;
    for (i = 0; i < l; i++) {
      fstr = _m_lang[i][fromLn];
      re = new RegExp(fstr);
      rstr = _m_lang[i][toLn];
...
//-----------------------------------------------------------------------
class ceMmChronicle {
  late final _m_ev, _m_rul, _m_dyn;

  //-------------------------------------------------------------------------
  constructor() {
    _m_ev = ceMmChronicle.InitChronicle();
    _m_rul = ceMmChronicle.InitRulers();
    _m_dyn = ceMmChronicle.InitDynasties();
  }
...
//output: (i= index)
  hSearch(jdn) {
    var i = 0;
    var l = 0;
    var u = _m_ev.length - 1;
    while (u >= l) {
      i = Math.floor((l + u) / 2); // calculate midpoint
      if (_m_ev[i]["Julian Day Number"] < jdn)
        l = i + 1;
      else if (_m_ev[i]["Julian Day Number"] > jdn)
        u = i - 1;
      else {
        return i;
      } // found
  ...
  static InitRulers() {
    return [
      ...
      },
      {
        ...
        "Dynasty": "British_Colonial_Period",
        "URL":
            "https://en.wikipedia.org/wiki/Alexander_Mackenzie_(civil_servant)"
      },
      {
  ...
  static InitChronicle() {
    return [
      ...
      },
      {
        ...
        "Day of the Week": "Friday",
        "Description":
            "Marriage between Minye Kyawhtin and Princess Thakin Gyi."
      },
      {
